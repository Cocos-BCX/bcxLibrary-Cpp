...
struct get_impacted_account_visitor
{
   ...
   void operator()( const fba_distribute_operation& op )
   {
    ...
    void operator()( const bcx::protocol::contract_create_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::contract_call_function_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_register_creator_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_create_world_view_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_relate_world_view_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_create_asset_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_relate_asset_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_delete_asset_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_transfer_asset_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_create_asset_order_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::nh_cancel_asset_order_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::contract_revise_operation& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }


    void operator()( const bcx::protocol::fill_nh_asset_order& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::create_file& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::add_file_relate_account& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::file_signature& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
    void operator()( const bcx::protocol::relate_parent_file& op )
    {
        _impacted.insert( op.fee_payer() ); // account_id
    }
