...
{
    joinWebSocketThread();
    BCX_SAFE_DELETE(_subThreadInstance);
...
//    auto fileUtils = FileUtils::getInstance();
    bool isCAFileExist = false; // fileUtils->isFileExist(_caFilePath);
//    if (isCAFileExist)
//    {
        ...
        if (iter != __wsHelper->_subThreadWsMessageQueue->end())
        {
            ...
            else
            {
                ...
                { // If frame initialization failed, delete the frame and drop the sending data
                    ...
                    delete frame;
                    BCX_SAFE_FREE(data->bytes);
                    BCX_SAFE_DELETE(data);
                    __wsHelper->_subThreadWsMessageQueue->erase(iter);
                    BCX_SAFE_DELETE(subThreadMsg);
                    break;
                }
            }
